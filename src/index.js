...
export const getFixedValue = (number, accuracy) => {
  const degree = Math.pow(10, accuracy )
  const fixedValue = Math.round(number * degree)
  return fixedValue / degree
};
 ...
 * @returns {number}
 */
export const randomizer = (from, to) => Math.floor(Math.random() * (to - from + 1)) + from;
